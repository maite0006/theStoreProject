...
namespace projectAPI.Controllers
{
    ...
    public class ProductoController : ControllerBase
    {
        ...
        public async Task<IActionResult> eliminarProd( Guid id)
        {
            try
            {
                var eliminado = await _cUBajaProd.EliminarProd(id);
                if (!eliminado)
                    return BadRequest("Se produjo un error al eliminar su producto");
                return Ok($"Prod {id} eliminado con exito");
        ...
        public async Task<IActionResult> AgregarProd([FromBody] AltaProdInDTO dto)
        {
            ...
            try
            {
                AltaProdOutDTO dtoOut = await _cUAltaProd.AgregarP(dto);
                return Ok(dtoOut);
